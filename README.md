...
A Swing-based desktop GUI is available in the `brut.apktool:apktool-gui` module. Run `./gradlew :brut.apktool:apktool-gui:run` during development or execute the shaded artifact generated by `./gradlew :brut.apktool:apktool-gui:shadowJar` to launch a graphical front end for decoding, building, and managing framework resources.
